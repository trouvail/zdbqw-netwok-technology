...
// 初始化日志的参数
int zyl_log::num = 0;
zyl_log zyl_log::diary[50] = {};
FILE* zyl_log::fp = nullptr;
zyl_log::zyl_log()
{
	// `追加 / 更新`方式打开文件 就是可以随意更新文件 将写入的数据追加到文件的末尾
	fp = fopen("zyl_log.txt", "a+");
}
zyl_log::~zyl_log()
{
	fclose(fp);
...
// `ip`类型写入日志
void zyl_log::write2log_ip(const char* a, Data_t* pkt)
{
	fprintf(fp, "`IP`");
	fprintf(fp, a);
	fprintf(fp, ": ");
...
// `arp`类型写入日志
void zyl_log::write2log_arp(ARPFrame_t* pkt)
{
	fprintf(fp, "ARP:");
